"""Sensitivity Analysis 3 (SI Appendix E) — Measurement-error specification.

Re-estimates the primary Lag-1 Hierarchical Bayesian Dynamic Panel Model
treating each observed country-year outcome as a noisy realization of a
latent value:

    y_obs[c,t] ~ Normal(y*[c,t], s[c,t])
    y*[c,t]    = mu[c,t] + sigma_y * z[c,t],   z ~ Normal(0, 1)   (non-centred)

with the observation-specific measurement SD s[c,t] set to half the
codelow-codehigh band returned by the V-Dem measurement model. The linear
predictor mu, the hierarchical structure, priors, and convergence thresholds
are inherited unchanged from ``fit_main_model_correct.py``.

Sampling: NUTS, 4 chains, tune=3000, draws=2000, target_accept=0.99
(higher tuning/acceptance to accommodate the latent-outcome geometry).

Outputs
-------
output/idata_sensitivity_measerr.nc
output/tables/posterior_summary_si_measerr_95hdi.csv   (Supplementary Table S3)
"""
...
def load_and_prepare() -> pd.DataFrame:
    ...
    needed = [
        OUTCOME, "v2smpolsoc_lag1", "p_score_mean_lag1", "stringency_mean_lag1",
        "v2smpolsoc_codelow", "v2smpolsoc_codehigh", *COVARIATES, REGION_COL,
    ]
...
def main() -> None:
    ...
    s_meas = ((df["v2smpolsoc_codehigh"] - df["v2smpolsoc_codelow"]) / 2.0).to_numpy(float)
    ...
    coords = {"country": country_labels, "region": region_labels,
              "covar": COVARIATES, "obs": np.arange(len(df))}
    with pm.Model(coords=coords):
        ...
        z_true = pm.Normal("z_true", 0.0, 1.0, dims="obs")
        y_true = pm.Deterministic("y_true", mu + sigma_y * z_true, dims="obs")
        pm.Normal("y", mu=y_true, sigma=s_meas, observed=y, dims="obs")

        idata = pm.sample(draws=2000, tune=3000, chains=4, cores=4,
                          target_accept=0.99, random_seed=SEED)

    idata.to_netcdf(OUT / "idata_sensitivity_measerr.nc")
    summary = az.summary(idata, hdi_prob=0.95,
                         var_names=["phi", "beta_p", "beta_s", "gamma",
                                    "mu_a", "sigma_region", "sigma_country", "sigma_y"])
    summary.to_csv(TABLES / "posterior_summary_si_measerr_95hdi.csv")
    ...
    print(f"mean measurement SD: {s_meas.mean():.3f} "
          f"(outcome cross-country SD: {df[OUTCOME].std(ddof=0):.3f})")
